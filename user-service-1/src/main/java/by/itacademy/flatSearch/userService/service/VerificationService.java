...
public class VerificationService implements IVerificationService {
    ...
    public void verify(VerificationDTO verificationDTO) {
        Optional<VerificationEntity> storedEntity = verificationDao.findByMailAndCode(verificationDTO.getMail(), verificationDTO.getCode());
        ...
        User user = crudUserDao.findByMail(storedEntity.get().getMail());
        user.setStatus(UserStatus.ACTIVATED);
        crudUserDao.save(user);

        verificationDao.delete(storedEntity.get());     // Delete user im mail Queue
    }
}
