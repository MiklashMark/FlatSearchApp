...
import by.itacademy.flatservice.core.RandomUserAgents;
import by.itacademy.parserservice.core.util.FindData;
import by.itacademy.parserservice.core.util.NumberUtils;
    ...
    private void findFlatsUrls(FindData findData) {
        ...
        for (int i = 0; i < findData.getPageCountForThread(); i++) {
            try {
                ...
                Elements links = document.select("a[href~=" + findData.getOfferType().getValue() + "\\d+] ");
                ...
                throw new InternalServerException("Ошибка в получении url из ресурса realt.by");
            }
        }
